...
from utils import query,extract_time
...
#%% PUBLIC METHODS    
    ...
    def get_rescuetime_log(self,start_date=None,end_date=None,store=None):
        ...
        if start_date is None:
            start_date = self._query_one("SELECT MAX(localdate) FROM rescuetime_log")[0]
        ...
        if store:
            self._store_df(df,'rescuetime_log')
        return df   
...
#        print('Status Code: ', resp.status_code) ### Need proper error handling here.
    ...
    def _resample5(self,df):
        df = df.resample('5T',how='sum')
        df = df[df.resample('5T',how='sum')['seconds']>90]
        df = pd.DataFrame(df)
